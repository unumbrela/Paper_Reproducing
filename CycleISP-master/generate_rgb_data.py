...
# 修改裁剪部分的代码（约第86行开始）
            ...
            # 确保裁剪范围有效
            if padh[j] < end_h and padw[j] < end_w:
                rgb_clean = rgb_clean[padh[j]:end_h, padw[j]:end_w, :]   
                rgb_noisy = rgb_noisy[padh[j]:end_h, padw[j]:end_w, :]   
            else:
                print(f"Warning: Invalid padding for {filename} - padh:{padh[j]}, padw:{padw[j]}, image size:{h}x{w}")

            # 确保图像尺寸有效
            if rgb_clean.shape[0] == 0 or rgb_clean.shape[1] == 0:
                print(f"Error: Zero-sized image after cropping for {filename}")
                continue
            ...
            imwrite(args.result_dir+'clean/'+filename[:-4]+'.png', img_as_ubyte(rgb_clean))
            imwrite(args.result_dir+'noisy/'+filename[:-4]+'.png', img_as_ubyte(rgb_noisy))